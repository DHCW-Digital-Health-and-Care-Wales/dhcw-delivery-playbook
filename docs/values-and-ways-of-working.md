...
This playbook is full of process: phases, roles, ceremonies, and engineering standards. None of that process is the point. It exists to express a set of values, and it only works when those values come first. This page sets out where we start, so that the rest of the playbook reads as the natural consequence of what we believe rather than a set of rules to follow.

The logic runs in one line. DHCW exists to improve health and care for the people of Wales. We can only do that well through teams of people, and people do their best work when they are led with compassion, feel safe to speak, and are trusted to organise their own work. Agile and Scrum give that trust a shape. Read the sections below in order, because each one depends on the one before it.
...
Our purpose is expressed through a small number of principles that shape how we prioritise and decide. They are the questions we return to when a trade-off is hard.
...
Agile is how we turn trust and safety into working software. It is a set of values before it is a set of practices. The four value statements from the Agile Manifesto describe where we place our emphasis. We value the items on both sides, and we lean towards the first in each pair.
...
Scrum gives these values a concrete home through five values that a healthy team lives out every day.

| Scrum value | How it shows up in our teams |
| --- | --- |
| **Commitment** | We commit to goals as a team and to supporting each other in reaching them. |
...
## Why the rest of this playbook looks the way it does

Read with these values in mind, the rest of the playbook stops being a list of rules and becomes a set of choices with reasons behind them.
